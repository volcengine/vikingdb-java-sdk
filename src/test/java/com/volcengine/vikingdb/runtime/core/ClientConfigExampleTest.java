...
public class ClientConfigExampleTest {
    ...
    public void testBuildClientConfigWithCustomRetryAndHttpClient() {
        ...
        cm.setDefaultMaxPerRoute(200);   // 单路由最大连接数，由于同一个client始终请求同一个host，必须与 MaxTotal 匹配
        cm.setValidateAfterInactivity(10000); // 空闲 10s 后的连接在复用前进行可用性检查

        // 3) 设置 超时配置：
        RequestConfig requestConfig = RequestConfig.custom()
                .setConnectionRequestTimeout(500) // 从连接池获取连接的超时(ms)：池满时最多等待多久
                .setConnectTimeout(1000)          // 建连超时(ms)：TCP 三次握手等建连过程的最大等待时间
                .setSocketTimeout(10000)           // 读超时(ms)：建立连接后等待响应数据的最大时间
                .build();
        ...
        CloseableHttpClient httpClient = HttpClients.custom()
                ...
                .evictIdleConnections(30, TimeUnit.SECONDS) // 开启后台线程,定期清理过期和空闲连接
                .build();
